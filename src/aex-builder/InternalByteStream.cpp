...
std::vector<std::uint8_t> &InternalByteStream::Bake()
{
    for (const Fixup &fixup : m_fixups) {
        ...
        ASSERT_MSG(it != m_labels.end(), "No label with fixup ID was found");
        ...
        for (size_t i = 0; i < sizeof(LabelPosition); i++) {
            ASSERT_MSG(m_stream[fixup_position + i] == ((uint8_t)-1), "Placeholder value in buffer should be set to ((uint8_t)-1)");
